...
const StarRating = ({ rating }) => {
    const stars = Array.from({ length: 5 }, (_, index) => (
        <StarIcon
            key={index}
            className={`h-5 w-5 ${index < rating ? 'text-green-400' : 'text-gray-300'}`}
        />
    ));
...
export default function ContactTestimonials() {
    return (
        <div className="bg-[var(--white)] text-black py-16 px-4 sm:px-8 lg:px-16 flex flex-col items-center">
            ...
            <div className="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <form className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" placeholder="Tu nombre" className="mt-1 block w-full rounded-md border-[var(--border)] shadow-sm focus:border-blue-500 focus:ring-[var(--color-primary)]" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Teléfono</label>
                                <input type="tel" placeholder="Tu teléfono" className="mt-1 block w-full rounded-md border-[var(--border)] shadow-sm focus:border-blue-500 focus:ring-[var(--color-primary)]" />
                            </div>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Correo electrónico</label>
                            <input type="email" placeholder="tu@email.com" className="mt-1 block w-full rounded-md border-[var(--border)] shadow-sm focus:border-blue-500 focus:ring-[var(--color-primary)]" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Servicio de interés</label>
                            <select className="mt-1 block w-full rounded-md border-[var(--border)] shadow-sm focus:border-blue-500 focus:ring-[var(--color-primary)]">
                        ...
                        <button
                            type="submit"
                            className="w-full py-3 px-4 text-[var(--color-primary)] text-[var(--white)] font-semibold rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:ring-opacity-50 transition-colors"
                        >
                ...
                >
                    ...
                    <div className="mt-12">
                        {/* Testimonios */}
                        <h2 className="text-xl font-bold text-slate-800 mb-4">Lo que dicen nuestros clientes</h2>
